...
// Helper: fully populate a post after save/update
const fullyPopulatePost = async (post) => {
  if (!post) return null;
  await post.populate('user', 'name username avatarUrl');
  await post.populate('comments.user', 'name username avatarUrl');
...
// Helper: format post safely for frontend
const formatPostForFrontend = (post) => {
  ...
  const author = postObj.user ? {
    id: postObj.user._id,
    name: postObj.user.name,
  ...
  const formattedComments = (postObj.comments || [])
    .map((c) => {
  ...
  return {
    ...
    timestamp: postObj.createdAt || new Date(),
    likes: (postObj.likes || []).map((id) => id.toString()),
...
router.get('/feed', protect, async (req, res) => {
  try {
    ...
    const formatted = posts.map(formatPostForFrontend).filter(Boolean);
    res.json(formatted);
  } catch (error) {
...
router.post('/', protect, async (req, res) => {
  ...
  try {
    ...
    let created = await post.save();
    created = await fullyPopulatePost(created);

    const formatted = formatPostForFrontend(created);
    req.io.emit('newPost', formatted);
...
router.put('/:id/like', protect, async (req, res) => {
  try {
    ...
    if (isLiked) {
      ...
      if (post.user.toString() !== req.user.id) {
        ...
        if (!existingNotification) {
          ...
          await notification.save();
          const populatedNotif = await notification.populate('sender', 'name username avatarUrl');
          const recipientSocket = req.onlineUsers.get(post.user.toString());
          if (recipientSocket) {
            req.io.to(recipientSocket).emit('newNotification', populatedNotif);
          }
        }
      }
    }
    ...
    updated = await fullyPopulatePost(updated);
    const formatted = formatPostForFrontend(updated);
    req.io.emit('postUpdated', formatted);
    res.json(formatted);
  } catch (error) {
...
router.post('/:id/comments', protect, async (req, res) => {
  ...
  try {
    ...
    if (post.user.toString() !== req.user.id) {
      ...
      await notif.save();
      const populatedNotif = await notif.populate('sender', 'name username avatarUrl');
      const recipientSocket = req.onlineUsers.get(post.user.toString());
      if (recipientSocket) {
        req.io.to(recipientSocket).emit('newNotification', populatedNotif);
      }
